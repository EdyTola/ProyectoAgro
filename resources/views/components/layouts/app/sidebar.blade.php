...
<div class="h-full py-4 px-3">
    <ul class="space-y-2 font-medium">
        {{-- Bloque principal para usuarios autenticados --}}
        @auth
            {{-- Lógica para mostrar enlaces específicos del Administrador --}}
            {{-- Muestra enlaces de admin SOLO si el usuario es 'administrador' Y está en la ruta 'admin.blank_page' --}}
            ...
            @endif {{-- Cierre del @if de rol/ruta --}}
        ...
        @endauth {{-- Cierre del @auth principal --}}

        {{-- Bloque para usuarios NO autenticados (invitados) --}}
        @guest
            <li>
                <a href="{{ route('home') }}" class="flex items-center p-2 text-base font-normal rounded-lg text-[#1b1b18] dark:text-white hover:bg-[#F7F6F3] dark:hover:bg-gray-700 {{ Request::routeIs('home') ? 'bg-[#F7F6F3] dark:bg-gray-700' : '' }}">
                    {{-- Icono de Inicio --}}
        ...
        @endguest
        {{-- Cerrar Sesión solo para usuarios autenticados, y fuera del bloque condicional de rol --}}
        @auth
            <li>
                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    @csrf
